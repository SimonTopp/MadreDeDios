...
```{r download, eval = F}
## Download data from Earth Engine pull
...
#Split file paths into water only pull and everything pull
...
## Bunch of stuff that's messy playing around with the data
check <- df %>%
  filter(red < 10000) %>%
...
##  Look at formation and infilling through ndvi/ndwi
...
## Mess around with water only pull
...
ggplot(water.filt %>% filter(red < 2500), aes(x = date, y = red, color = season)) +
  geom_point(alpha = .5,size = 1) +
  geom_smooth(se = F, method = 'lm', span = 50) +
  #ylim(0,5000) +
  labs(y = 'Red', title = 'Sediment Proxy by Season') +
  facet_wrap(~OrderUp, scales = 'free')

ggsave('figures/sedimentBySeason.png')

water.filt %>%
  group_by(OrderUp, season, year) %>%
  summarize(count = n()) %>%
  ggplot(., aes(x = season, y = count)) + 
  geom_boxplot() +
  theme_bw() +
  ggtitle('Overpass counts of oxbows per season')
  
  group_by(season, OrderUp) %>%
...
ggsave('figures/SeasonOverpassCount.png', height = 4, width = 4, units = 'in')
