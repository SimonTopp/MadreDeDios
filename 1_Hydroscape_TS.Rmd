...
basin <- read.csv('out/Basin_Deforestation_Lagged.csv') %>%
  ...
  mutate(Basin = ifelse(BasinID == 6080540660, 'Upstream Manu', #'Arriba de Manu',
                        ifelse(BasinID == 6080548660,'Conservation Area', #'Área de conservación',
                               ifelse(BasinID == 6080540560, 'UpstreamManuNorth',
                                ifelse(BasinID == 6080549240, 'Manu - Co.',
                                  ifelse(BasinID == 6080549470, 'Colorado',
                                   ifelse(BasinID == 6080548950, 'Co.- In. West',
...
leg <- cowplot::get_legend(ggplot(basin, aes(x = year, y = cumulative)) + 
  ...
  #scale_fill_manual('Tipo de\nConversión', values = c('#5d4235','#1e96a6'), labels = c('A Estéril', 'A Agua')))
...
#g <- gridExtra::grid.arrange(p1, p2, leg, ncol = 3, widths = c(1,1,.5), left = grid::textGrob(bquote('Pérdida acumulada de bosques ('~km^2*')'), rot = 90), bottom = 'Año', top = 'Pérdida de Bosques por Cuenca') 

ggsave('figures/Final/cumForestLoss_wRiv.png', g, height = 8, width = 5, units = 'in', dpi = 600)
...
ggmap::ggmap(sq_hybrid, darken = c(0.1, "white")) + 
  ...
  ggthemes::theme_map() + 
  theme(legend.position = c(0.7,0.55)) + 
  scale_color_manual(values = c('red', colors),
...
ggsave('figures/Final/SampleSitesv2.png', width = 4, height = 4, units ='in', dpi = 600)
