...
```{r}
drive_download('MdD_Exports/MdD_Hydroscape_TS_DL_Riv.csv', 'out/MdD_Hydroscape_TS_DL_Riv.csv', overwrite = T)
...
hs <- read_csv('out/MdD_Hydroscape_TS_DL_Riv.csv') %>% #('out/MdD_Hydroscape_TS_2yearv2.csv') %>% #
  select(-`system:index`) %>%
  spread(feature, area) %>%
...
ggplot(hs %>% filter(!feature %in% c('cl', 'waterArea')), aes(x = year, y = area.percent, color = feature)) + 
  ...
  facet_wrap(~Basin) +
  theme_bw() +
  labs(x = 'Year', y = 'Normalized Area (Area/mean Area)', color = 'Type', title = 'Change in Surface water extent')#+ coord_cartesian(ylim = c(-2,2))
...
ggplot(basin, aes(x = year, y = cumulative, fill = feature)) + geom_col() + facet_wrap(~Basin) + labs(x = 'year', y = 'Forest loss (sq km)', title = 'Forest loss within the Basin Same year')



ggplot(basin %>% filter(feature == 'ForestToWater'), aes(x = year, y = area.km)) + geom_col() + facet_wrap(~Basin) + labs(x = 'year', y = 'Forest loss (sq km)', title = 'Forest loss within the Basin')
ggplot(basin %>% filter(feature == 'ForestToWater'), aes(x = year, y = cumulative)) + geom_col() + facet_wrap(~Basin) + labs(x = 'year', y = 'Forest loss (sq km)', title = 'Forest loss within the Basin Same year')
