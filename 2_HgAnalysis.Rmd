...
level.order<-c("river.upstream", "oxbow.lake.upstream", "mining.pond","river.downstream","oxbow.lake.downstream")
...
## Total Hg
g1<-ggplot(water.all,aes(x=factor(Site.type,level=c("river.upstream", "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream")), y=THg.water.ng.L, fill=Site.type)) + 
...
## MeHg
g2<-ggplot(water.all,aes(x=factor(Site.type,level=c("river.upstream",  "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream")), y=MeHg.water.ng.L, fill=Site.type)) + 
  geom_boxplot() + 
  theme_classic()+
  labs(x="",y="MeHg (ng/L)")+
  theme(axis.text.x=element_blank())+
...
## Percent MeHg
g3<-ggplot(water.all,aes(x=factor(Site.type,level=c("river.upstream", "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream")), y=Percent.MeHg.water,fill=Site.type))+
  geom_boxplot()+
  theme_classic()+
  labs(x="",y="Percent Hg\nas MeHg")+
  scale_fill_manual(values=cbp1)+
...
g4<-ggplot(water.all,aes(x=factor(Site.type,level=c("river.upstream", "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream")), y=Ave.THg.sed.ug.g,fill=Site.type))+
  geom_boxplot()+
  theme_classic()+
...
#Total Hg
gg1<-ggplot(water.all.THg, aes(x=ID, y=THg.water.ng.L, fill=factor(Site.type, level=c("river.upstream", "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream"))))+
...
# MeHg
gg2<-ggplot(water.all.MeHg, aes(x=ID, y=MeHg.water.ng.L,  fill=factor(Site.type,level=c("river.upstream", "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream"))))+
  geom_bar(stat="identity")+
  theme_classic()+
  labs(x="",y="")+
  theme(axis.text.x=element_blank())+
...
## Percent MeHg
gg3<-ggplot(water.all.Percent, aes(x=ID, y=Percent.MeHg.water, fill=factor(Site.type,level=c("river.upstream", "oxbow.lake.upstream", "mining.pond", "river.downstream", "oxbow.lake.downstream"))))+
  geom_bar(stat="identity")+
  theme_classic()+
  labs(x="\ncount",y="",fill="")+
  theme(axis.text.x=element_blank())+
...
gg4<-ggplot(water.all.THg.sed,aes(x=ID,y=Ave.THg.sed.ug.g,fill=factor(Site.type,level=c("river.upstream", "oxbow.lake.upstream", "mining.pond","river.downstream","oxbow.lake.downstream"))))+geom_bar(stat="identity")+theme_classic()+labs(x="\ncount",y="")+theme(axis.text.x=element_blank())+labs (fill="\ncount")+scale_fill_manual (values=cbp2)

#Combine plots into a water column figure and sediment figure
library(ggpubr)
#Water column figure
ggarrange(g1,gg1,g2,gg2,g3,gg3,nrow=3,ncol=2,common.legend=TRUE)

#Sediment figure
ggarrange(g4,gg4,nrow=1,ncol=2,common.legend=TRUE)
...
library(dplyr)
water.all %>%  group_by(Site.type) %>% summarize(n=n(),mean_THg = mean(THg.water.ng.L, na.rm = TRUE),sd_THg=sd(THg.water.ng.L, na.rm = TRUE),mean_MeHg=mean(MeHg.water.ng.L,na.rm=TRUE),sd_MeHg=sd(MeHg.water.ng.L, na.rm = TRUE),mean_Percent=mean(Percent.MeHg.water,na.rm=T),sd_Percent=sd(Percent.MeHg.water, na.rm = TRUE))
...
library(FSA)
dunnTest(THg.water.ng.L ~ Site.type, data = water.all)
...
dunnTest(MeHg.water.ng.L ~ Site.type, data = water.all)
...
dunnTest(Percent.MeHg.water ~ Site.type, data = water.all)
...
dunnTest(Ave.THg.sed.ug.g ~ Site.type, data = water.all)
