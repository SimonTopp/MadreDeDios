...
# water.labels <- c("Río Arriba", "Cocha Arriba", "Estanque Minero", "Río Abajo", "Cocha Abajo")
# fig.labels <- c("Río\nArriba","Cocha\nArriba","Estanque\nMinero","Río\nAbajo", "Cocha\nAbajo")
...
p1 <- ggplot(water.all, aes(x=TSS.mg.L, y=THg.water.ng.L))+
  ...
  labs(y="THg (ng/L)",x="TSS (mg/L)",color="") +
...
## TSS vs MeHg
check <- water.all %>% mutate(type = ifelse(Site.type %in% c('Río Arriba', 'Río Abajo'), 'lotic', 'lentic'))

p2 <- ggplot(check, aes(x=TSS.mg.L, y=MeHg.water.ng.L, color = type))+
  geom_point(size=2.5,alpha=.7)+
  #labs(y="MeHg (ng/L)",x="Total Suspended Solids (mg/L)",color="") +
  labs(y="MeHg (ng/L)",x="TSS (mg/L)",color="") +
  scale_color_manual(values=cb) +
  scale_x_continuous(trans = 'log10') +
  scale_y_continuous(trans = 'log10') +
  ggpmisc::stat_fit_glance(method = 'lm',
...
g <- ggpubr::ggarrange(p1, p2,nrow=1,ncol=2,common.legend=TRUE)
g
